...
fn style_uses_numeric_custom_properties(style: &Style) -> bool {
    ...
    let width_uses_var = style
        .width_value()
        .is_some_and(|width| matches!(width.size(), Some(twill_core::utilities::Size::Var(_))));
    let height_uses_var = style.height_value().is_some_and(|height| {
        matches!(
            height.size(),
            Some(twill_core::utilities::Size::HeightVar(_))
        )
    });
