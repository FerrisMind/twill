...
fn test_font_size_resolution_variants() {
    const TITLE_SIZE: twill_core::tokens::FontSizeVar =
        twill_core::tokens::FontSizeVar::new("--title");
...
fn test_style_padding_allows_partial_sides() {
    let px = to_style_padding(twill_core::utilities::Padding::x(Spacing::S4), &[]);
...
fn test_style_padding_logical_aliases_map_to_physical_sides() {
    let ps = to_style_padding(twill_core::utilities::Padding::ps(Spacing::S6), &[]);
    let pe = to_style_padding(twill_core::utilities::Padding::pe(Spacing::S6), &[]);
    let pbs = to_style_padding(twill_core::utilities::Padding::pbs(Spacing::S2), &[]);
    let pbe = to_style_padding(twill_core::utilities::Padding::pbe(Spacing::S2), &[]);
...
fn test_style_padding_px_token_families() {
    let p = to_style_padding(twill_core::utilities::Padding::p(Spacing::Px), &[]);
    let px = to_style_padding(twill_core::utilities::Padding::px(Spacing::Px), &[]);
    let py = to_style_padding(twill_core::utilities::Padding::py(Spacing::Px), &[]);
    let pt = to_style_padding(twill_core::utilities::Padding::pt(Spacing::Px), &[]);
    let pr = to_style_padding(twill_core::utilities::Padding::pr(Spacing::Px), &[]);
    let pb = to_style_padding(twill_core::utilities::Padding::pb(Spacing::Px), &[]);
    let pl = to_style_padding(twill_core::utilities::Padding::pl(Spacing::Px), &[]);
    let ps = to_style_padding(twill_core::utilities::Padding::ps(Spacing::Px), &[]);
    let pe = to_style_padding(twill_core::utilities::Padding::pe(Spacing::Px), &[]);
    let pbs = to_style_padding(twill_core::utilities::Padding::pbs(Spacing::Px), &[]);
    let pbe = to_style_padding(twill_core::utilities::Padding::pbe(Spacing::Px), &[]);
...
fn test_style_padding_custom_property_and_arbitrary_values() {
    let var = twill_core::utilities::PaddingVar::new("--pad");
    let p = twill_core::utilities::Padding::individual_value(
...
fn test_resolve_width_fixed_and_container_variants() {
    assert_eq!(
        resolve_width(twill_core::utilities::Width::w(Spacing::S24), &[]),
        Some(ResolvedWidth::Length(Length::Fixed(96.0)))
    );
    assert_eq!(
        resolve_width(
            twill_core::utilities::Width::w_container(twill_core::tokens::Container::Md),
            &[]
        ),
        Some(ResolvedWidth::Length(Length::Fixed(448.0)))
    );
    assert_eq!(
        resolve_width(twill_core::utilities::Width::w_px(), &[]),
        Some(ResolvedWidth::Length(Length::Fixed(1.0)))
    );
    assert_eq!(
        resolve_width(twill_core::utilities::Width::w_full(), &[]),
        Some(ResolvedWidth::Length(Length::Fill))
    );
    assert_eq!(
        resolve_width(twill_core::utilities::Width::w_auto(), &[]),
...
fn test_resolve_width_fraction_variants() {
    let half = resolve_width(
        twill_core::utilities::Width::w_fraction(twill_core::tokens::Percentage::S1_2),
        &[],
    );
    match half {
    ...
    let two_fifths = resolve_width(
        twill_core::utilities::Width::w_fraction(twill_core::tokens::Percentage::S2_5),
        &[],
    );
    match two_fifths {
...
fn test_resolve_width_custom_property_and_px_value() {
    let var = twill_core::utilities::WidthVar::new("--panel-w");
    assert_eq!(
        resolve_width(
            twill_core::utilities::Width::w_var(var),
            &[("--panel-w", 280.0)]
        ),
        Some(ResolvedWidth::Length(Length::Fixed(280.0)))
    );
    assert_eq!(
        resolve_width(twill_core::utilities::Width::w_px_value(320), &[]),
...
fn test_resolve_height_fixed_and_variant_families() {
    assert_eq!(
        resolve_height(twill_core::utilities::Height::h(Spacing::S24), &[]),
        Some(ResolvedHeight::Length(Length::Fixed(96.0)))
    );
    assert_eq!(
        resolve_height(twill_core::utilities::Height::h_px(), &[]),
        Some(ResolvedHeight::Length(Length::Fixed(1.0)))
    );
    assert_eq!(
        resolve_height(twill_core::utilities::Height::h_full(), &[]),
        Some(ResolvedHeight::Length(Length::Fill))
    );
    assert_eq!(
        resolve_height(twill_core::utilities::Height::h_auto(), &[]),
        Some(ResolvedHeight::Length(Length::Shrink))
    );
    assert_eq!(
        resolve_height(twill_core::utilities::Height::h_lh(), &[]),
        Some(ResolvedHeight::Length(Length::Shrink))
    );
}
...
fn test_resolve_height_fraction_variants() {
    let half = resolve_height(
        twill_core::utilities::Height::h_fraction(twill_core::tokens::Percentage::S1_2),
        &[],
    );
    match half {
    ...
    let two_fifths = resolve_height(
        twill_core::utilities::Height::h_fraction(twill_core::tokens::Percentage::S2_5),
        &[],
    );
    match two_fifths {
...
fn test_resolve_height_custom_property_and_px_value() {
    let var = twill_core::utilities::HeightVar::new("--panel-h");
    assert_eq!(
        resolve_height(
            twill_core::utilities::Height::h_var(var),
            &[("--panel-h", 240.0)]
        ),
        Some(ResolvedHeight::Length(Length::Fixed(240.0)))
    );
    assert_eq!(
        resolve_height(twill_core::utilities::Height::h_px_value(360), &[]),
...
fn test_shadow_uses_custom_color() {
    let shadow = to_shadow_with_color(
        Shadow::Sm,
        twill_backend_common::ShadowColor::Explicit(Color::red(Scale::S500)),
    );
...
fn test_shadow_layers_for_sm_are_two() {
    let layers =
        to_shadow_layers_with_color(Shadow::Sm, twill_backend_common::ShadowColor::Default);
...
fn test_shadow_layers_for_2xl_are_single() {
    let layers =
        to_shadow_layers_with_color(Shadow::S2xl, twill_backend_common::ShadowColor::Default);
