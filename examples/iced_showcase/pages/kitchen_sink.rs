...
use twill::iced::{
    danger_button, primary_button, secondary_button, styled_button, styled_container, to_color,
    to_font_size, to_font_weight,
};
...
pub fn view<'a>(is_dark: bool) -> Element<'a, Message> {
    ...
    let mut color_col = column![].spacing(8);
    let mut color_row = row![].spacing(6);
    let mut color_count = 0;
    for color in color_families {
        color_row = color_row.push(styled_container(
            Space::new()
        ...
        color_count += 1;
        if color_count == 6 {
            color_col = color_col.push(color_row);
            color_row = row![].spacing(8);
            color_count = 0;
        }
    }
    if color_count > 0 {
    ...
    let typo = column![
        text("Heading 1")
            .size(to_font_size(FontSize::S3xl))
            .font(iced::Font {
                weight: to_font_weight(FontWeight::Bold),
                ..Default::default()
            }),
        text("Heading 2")
            .size(to_font_size(FontSize::S2xl))
            .font(iced::Font {
    ...
    for r in radii {
        radius_row = radius_row.push(styled_container(
            Space::new()
    ...
    // ── Shadow samples ──────────────────────────────────────────────
    let box_bg_twill = if is_dark {
        Color::gray(Scale::S800)
    } else {
        Color::white()
    };
    let shadow_col_twill = if is_dark {
        Color::black()
    } else {
    ...
    let mut shadow_col = column![].spacing(16);
    let mut shadow_row = row![].spacing(24).padding(16);
    let mut shadow_count = 0;
    for shadow in shadow_variants {
        shadow_row = shadow_row.push(styled_container(
            Space::new()
        ...
        shadow_count += 1;
        if shadow_count == 3 {
            shadow_col = shadow_col.push(shadow_row);
            shadow_row = row![].spacing(24).padding(16);
            shadow_count = 0;
        }
    }
    if shadow_count > 0 {
    ...
    let left_col = column![
        section("Color Palette", color_col, text_col),
    ...
    let right_col = column![
        ...
        section("Shadows", shadow_col, text_col),
        section("Spacing (Padding)", spacing_row, text_col),
    ]
    .spacing(20)
    .width(Length::FillPortion(1));

    let responsive_grid = row![left_col, right_col].spacing(20).width(Length::Fill);

    // ── Assemble kitchen sink ───────────────────────────────────────
    column![
        text("Kitchen Sink").size(32).font(iced::Font {
            weight: to_font_weight(FontWeight::Bold),
            ..Default::default()
        }),
        text("All token categories at a glance for visual regression testing.")
            .size(16)
            .color(subtle_text),
        container(responsive_grid).width(Length::Fill),
    ]
    .spacing(24)
    .into()
}
