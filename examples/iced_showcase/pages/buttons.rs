...
use crate::Message;
use twill::iced::{twill_button, to_font_weight};
use twill::tokens::FontWeight;
use twill::components::Button as TwillButton;

pub fn view<'a>(is_dark: bool) -> Element<'a, Message> {
    column![
        text("Buttons").size(32).font(iced::Font { weight: to_font_weight(FontWeight::Bold), ..Default::default() }),
...
let icon = TwillButton::outline().icon();"#;
    ...
    let content = row![
        column![text("Small").size(12), twill_button(&btn_sm, "Button", Message::NoOp)].spacing(8).align_x(iced::Alignment::Center),
        column![text("Medium").size(12), twill_button(&btn_md, "Button", Message::NoOp)].spacing(8).align_x(iced::Alignment::Center),
        column![text("Large").size(12), twill_button(&btn_lg, "Button", Message::NoOp)].spacing(8).align_x(iced::Alignment::Center),
        column![text("Icon").size(12), twill_button(&btn_icon, "⌘", Message::NoOp)].spacing(8).align_x(iced::Alignment::Center),
    ].spacing(32).align_y(iced::Alignment::End);
...
let full = TwillButton::secondary().full_width(true);"#;
    ...
    let full_width_demo = container(
        twill_button(&btn_full, "Full Width Button", Message::NoOp)
