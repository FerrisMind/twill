...
use crate::Message;
use twill::tokens::{BorderRadius, BorderWidth, BorderStyle, Color, Scale, Shadow, Spacing, FontWeight};
use twill::style::Style;
use twill::utilities::Padding;
use twill::iced::{styled_container, twill_button, to_color, to_font_weight};
use twill::components::Button as TwillButton;

pub fn view<'a>(is_dark: bool) -> Element<'a, Message> {
    column![
        text("Style Builder & Components").size(32).font(iced::Font { weight: to_font_weight(FontWeight::Bold), ..Default::default() }),
...
styled_container(content, &style)"#;
    ...
    let card1 = styled_container(
        text("Primary Card").size(16).color(to_color(Color::white())).into(),
        &style1,
    );

    // Card 2: Subtle card
    let bg2 = if is_dark { Color::gray(Scale::S800) } else { Color::gray(Scale::S100) };
    let style2 = Style::new()
        .bg(bg2)
        .rounded(BorderRadius::Xl)
        .shadow(Shadow::Sm)
        .padding(Padding::all(Spacing::S6))
        .border(BorderWidth::S1, BorderStyle::Solid, if is_dark { Color::gray(Scale::S700) } else { Color::gray(Scale::S300) });

    let card2 = styled_container(
        column![
            text("Subtle Card").size(16).color(text_col),
            Space::new().width(Length::Shrink).height(Length::Fixed(4.0)),
            text("With border and shadow").size(12).color(to_color(Color::gray(Scale::S400))),
        ].into(),
        &style2,
    );
    ...
    let card3 = styled_container(
        text("Danger Card").size(16).color(to_color(Color::white())).into(),
        &style3,
    );
    ...
    let card4 = styled_container(
        column![
            text("Success Card").size(18).color(to_color(Color::white())).font(iced::Font { weight: to_font_weight(FontWeight::Bold), ..Default::default() }),
            Space::new().width(Length::Shrink).height(Length::Fixed(4.0)),
            text("Elevated with large shadow").size(13).color(to_color(Color::gray(Scale::S200))),
        ].into(),
        &style4,
    );
